...
<div class="mod-opening-hours" data-current-week-index="{{ $currentWeekIndex }}" data-total-weeks="{{ $totalWeeks }}" data-closed-label="{{ __('Closed', 'modularity-opening-hours') }}">
    @if (!empty($weeks))
        <script type="application/json" class="mod-opening-hours__data">{!! $weeksJson !!}</script>
        <div class="mod-opening-hours__week-header">
            <h3 class="mod-opening-hours__week-title">{{ $currentWeek['weekLabel'] ?? '' }}</h3>
